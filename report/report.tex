...
\begin{figure}[ht]
  ...
  NFE$=5$ (Wasserstein-1 $=0.77$) and monotonically improves to $0.10$
  at NFE$=50$. DDIM sampling from the DDPM checkpoint diverged at every
  sub-1000 NFE we tested, returning Wasserstein-1 $\sim 10^{14}$ (right
  panel uses a log scale; the apparent flat lines are converged FM
  values vs.\ saturated DDPM-divergence values, not a numerical
  artefact). The comparison should therefore be read as ``FM remains
  usable when DDPM is not'' rather than as a quantitative trade-off
  curve for DDPM.}
  \label{fig:nfe}
\end{figure}
