...
Diffusion Baseline and Flow Matching Variant\thanks{Code and trained
checkpoints: \url{https://github.com/qilinxiaoxiang/24788-kolmogorov-flow}}}
...
\begin{figure}[ht]
  ...
  \caption{Qualitative samples at default NFE (DDPM 1000, FM 50).
  Flow Matching reproduces the paired-vortex dipole structure of the real
  fields, whereas DDPM produces samples dominated by extreme-value outliers.}
  \label{fig:samples}
\end{figure}
...
\begin{figure}[ht]
  ...
  spectrum across wavenumbers; DDPM over-concentrates energy at all scales,
  consistent with its larger variance in generated fields.}
  \label{fig:spectrum}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption{Marginal vorticity PDF. Flow Matching matches the real tails;
  DDPM has heavy tails driven by the outlier samples.}
  \label{fig:pdf}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption{Sampling efficiency vs.\ NFE. Flow Matching is stable from
  NFE$=5$ and monotonically improves; DDIM from the DDPM checkpoint diverges
  for every NFE $<1000$ tested (Wasserstein-1 $\sim 10^{14}$,
...
\begin{table}[ht]
  ...
  at full scale (lower is better). \textit{Numbers filled in from
